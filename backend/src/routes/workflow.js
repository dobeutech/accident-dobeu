...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(`Workflow step ${stepId} updated for report ${reportId}`, {
...
router.post(
  '/:reportId/validate-photos',
  [authenticate, requirePermission('reports', 'write'), enforceFleetContext],
  async (req, res) => {
    try {
      const { reportId } = req.params;
...
router.get(
  ...
  [authenticate, requirePermission('override', 'read'), enforceFleetContext],
  async (req, res) => {
    try {
      const fleet_id = req.user.fleet_id;
      const supervisor_id =
        req.user.role === 'fleet_admin' || req.user.role === 'fleet_manager'
          ? req.user.userId
          : null;
...
router.post(
  '/override-requests/:requestId/deny',
  [
    authenticate,
    requirePermission('override', 'approve'),
    enforceFleetContext,
